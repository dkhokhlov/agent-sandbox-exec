...
| `agent-sandbox-execd` (root, systemd) | Loads + attaches the BPF program, owns `deny_map` (path→inode from the denylist), creates + delegates the agent cgroup, and migrates launcher requests into it. Watches the denylist + secret files, re-resolves on change. |
| `agent-sandbox-exec` (user) | Launcher: asks `agent-sandbox-execd` to migrate its pid into the cgroup, then execs the real agent. Unprivileged. |

Scope is a **cgroup**: the agent joins `/sys/fs/cgroup/agent-sandbox-exec`, which
...
subtree), so the launcher drops a request in `/run/agent-sandbox-exec/req` and
`agent-sandbox-execd` (root) performs the migration. No setuid binary; the daemon is the
...
Edit `/etc/agent-sandbox-exec/denylist` (a conffile — survives upgrades):
```
# one absolute path to a regular FILE per line; no dirs, no globs
/home/owner/.ssh/id_rsa
/home/owner/.aws/credentials
```
`agent-sandbox-execd` watches the file (and each secret) and re-resolves on change — no
...
```
agent-sandbox-exec sh -c 'cat /proc/self/cgroup'   # -> 0::/agent-sandbox-exec
echo /tmp/secret | sudo tee -a /etc/agent-sandbox-exec/denylist
...
- **Fail-closed launcher.** If `agent-sandbox-execd`/the cgroup/req-dir aren't ready, the
  launcher refuses to start the agent. `AGENT_SANDBOX_INSECURE=1` overrides.
- **Enforcement = daemon alive.** The BPF is attached while `agent-sandbox-execd` runs
