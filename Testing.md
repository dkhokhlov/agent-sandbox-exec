...
- Agent user: `owner` (uid 1000). Daemon: `sandboxd` as root via systemd
  `Type=notify`.
...
- Important: the Claude session running these tests was itself launched through
  ...
  `agent-sandbox` cgroup**. cgroup membership is inherited at fork, so a
...
| B2 | Deb file list | `dpkg-deb -c …deb` | unit under `/usr/lib/systemd/system`, no `/etc/systemd/system` | unit at `/usr/lib/systemd/system/agent-sandbox.service`; no `/etc/systemd/system` unit | PASS (M9) |
| B3 | Deb conffiles | `dpkg-deb --info …deb conffiles` | only `/etc/agent-sandbox/denylist` | `/etc/agent-sandbox/denylist` only | PASS (M9) |
...
| B7 | Unit fragment path after upgrade | `systemctl show -p FragmentPath` | `/usr/lib/systemd/system/agent-sandbox.service` | `/usr/lib/systemd/system/agent-sandbox.service` | PASS (M9) |

## C1 — cgroup escape (root-owned cgroup)

| # | Test | Command | Expected | Actual | Result |
|---|------|---------|----------|--------|--------|
| C1.1 | Cgroup dir ownership | `ls -ld /sys/fs/cgroup/agent-sandbox` | `root root` 0755 | `drwxr-xr-x root root` | PASS |
| C1.2 | `cgroup.procs` ownership | `ls -l …/cgroup.procs` | `root root` (reclaimed from prior delegated build) | `-rw-r--r-- root root` | PASS |
| C1.3 | Owner cannot mkdir child cgroup | `mkdir /sys/fs/cgroup/agent-sandbox/evade` | Permission denied | `mkdir: … Permission denied`, rc 1 | PASS |
...
| # | Test | Command | Expected | Actual | Result |
|---|------|---------|----------|--------|--------|
| C2.1 | Launcher migration via daemon request-dir | `/usr/bin/agent-sandbox-exec sh -c 'cat /proc/self/cgroup'` | process in `0::/agent-sandbox`, rc 0 | `0::/agent-sandbox`, rc 0 | PASS |
...
|---|-------------|---------|--------|
| O1 | Claude session is in the sandbox cgroup | `cat /proc/$claude_pid/cgroup` | `0::/agent-sandbox` |
...
- Deny is cgroup-scoped: blocked inside `agent-sandbox`, readable outside it.
